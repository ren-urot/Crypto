...
const TABS = ["About", "Price", "Performance", "Reviews", "News", "Guides"] as const;
      ...
      {questions.map((item, index) => {
        ...
        return (
          <div key={item.q} className="rounded-2xl bg-white p-5">
            ...
            {isOpen && (
              <p className="mt-3 text-sm leading-relaxed text-[#2d2d2d]">
                {item.a}
              </p>
            )}
          </div>
...
export default function CoinDetailView({ coin }: { coin: Coin }) {
  ...
  const recentPrices = [
    ...
    { label: "7 days", change: getHistoricalChange(coin.id, "7d") },
    { label: "30 days", change: getHistoricalChange(coin.id, "30d") },
    { label: "1 year", change: getHistoricalChange(coin.id, "1y") },
  ];
  ...
  return (
    <div>
      ...
      <div className="mt-6 flex gap-8 border-b border-[#e5e5e5]">
        {TABS.map((label) => (
          <button
            key={label}
            type="button"
      ...
      {tab === "About" && (
        <>
          <div className="mt-6 rounded-[20px] bg-white p-6">
            ...
            <p className="mt-3 max-w-[700px] text-base leading-relaxed text-[#2d2d2d]">
      ...
      {tab !== "Price" && tab !== "About" && (
        <p className="mt-6 text-sm text-[#929292]">{tab} coming soon.</p>
      )}
    </div>
  );
}
